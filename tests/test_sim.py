...
def test_sim(do_plot=False, do_save=False, do_run=True, **kwargs): # If being run via pytest, turn off
    ...
    # Figure out how to make it a single par dict

    # Option 1
    sim = hpv.Sim(
        sim_pars=sim_pars,  # Simulation parameters
        hpv_pars=hpv_pars,  # HPV parameters
        genotypes=genotypes,  # List of genotypes to include
        genotype_pars=genotype_pars,  # Dictionary of genotype-specific parameters
    )

    # Option 2: Flattish dict of pars that gets unpacked
    # & assigned to modules
    sim = hpv.Sim(beta=0.05, start=2020, sev_rate=dict(hpv16=0.2))
