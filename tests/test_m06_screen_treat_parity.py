"""M06 screen-treat cascade parity gate: multi-seed mean z-score vs v2 baseline.

Same gate pattern as test_m05_vx_routine_parity.py (10 v3 seeds vs 30 v2
seeds; |z| < 3 per metric). Metric set: four cascade scalars at end-of-sim
(n_screened_2060, n_screens_2060, n_cin_treated_2060, n_cin_treatments_2060)
plus cancer incidence and cancer deaths over [2030, 2060).

Requires the locally-regenerated v2 baseline at
``tests/regression/v2_seeds_n30_screen_treat.json``. See
``tests/regression/multi_seed_v2_screen_treat.py`` for how to (re)generate it
...
from tests.regression.anchor_screen_treat import build_v3_sim


BASELINE_PATH = Path(__file__).parent / 'regression' / 'v2_seeds_n30_screen_treat.json'
N_V3_SEEDS = 10
Z_THRESHOLD = 3.0
...
def _run_v3_seeds(n, start_seed=0):
    ...
    for seed in range(start_seed, start_seed + n):
        ...
        # --- cascade scalars at end-of-sim (2060) ---
        # BoolArr.sum() and FloatArr.sum() in v3 automatically exclude dead
        # agents — matching v2's alive-masked people.screened[alive].sum().
        primary = sim.interventions['primary']
        excision = sim.interventions['excision_rx']

        row['n_screened_2060'] = int(primary.screened.sum())
        row['n_screens_2060'] = int(primary.screens.sum())
        row['n_cin_treated_2060'] = int(excision.cin_treated.sum())
        row['n_cin_treatments_2060'] = int(excision.cin_treatments.sum())
        ...
        # --- cancer deaths over [2030, 2060) ---
        row['cancer_deaths_2030_2060'] = float(
            sim.results.hpvtotal.new_cancer_deaths[mask].sum()
        )
...
def test_m06_screen_treat_short_summary_parity():
    """6-metric z-score gate: |z| < 3 across screen-treat cascade scalars."""
    if not BASELINE_PATH.exists():
        pytest.skip(
            f'v2 baseline JSON not present — regenerate locally via '
            f'tests/regression/multi_seed_v2_screen_treat.py from a v2.3 env. '
