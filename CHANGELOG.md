...
- Adds **multiscale agents** via the `ms_agent_ratio` parameter (`hpv.Sim(ms_agent_ratio=N)`; default `1` = feature off). Ports v2's multiscale technique to the v3/Starsim architecture as a **cancer-pathway ledger**: each CIN agent's cancer pathway is resolved at `N` sub-resolutions — the agent's own cancer (counted at weight `1/N`) plus `N−1` EXTRA sub-cancers resolved as scheduled DATA in a deterministic side-RNG ledger (no fine People agents are grown). So the rare cancer-pathway events (causal infection, CIN2+, cancer) carry `N`× more samples per run for better-resolved age distributions (methods-manuscript Fig 5) at low agent count.
  - The ledger is a **read-only overlay**: because no agent state is grown, shrunk, or reweighted, the epidemic trajectory (transmission, demographics, every population result) is **bit-identical across `ms_agent_ratio`**. The extra sub-cancers are realized into the cancer results (`new_cancers`/`new_cancer_deaths` and their age sums) at their scheduled onset/death step, gated on the source agent being "available" (a shared competing-risk proxy for background death / emigration). The gate EXCLUDES the source's own cancer death — a coarse agent stands for `ratio` different people, so the source dying of its own cancer must not suppress a sibling sub-individual's independent cancer. Each extra's pre-CIN duration is rejection-sampled from the CIN-conditional distribution so it is an independent CIN2+-age sample.
  - *Regression information:* none at `ms_agent_ratio=1` — bit-identical to the prior code (verified: own cancer at weight 1.0, empty ledger). `ms_agent_ratio>1` is reproducible (the ledger uses a seed/step/genotype-keyed `numpy` stream independent of the slot CRN) and leaves the population bit-identical; only the cancer-pathway results gain resolution.
  - Scope/known residuals (documented in the design doc and `tests/test_multiscale_distribution.py`): multiscaling covers the cancer pathway only; causal-infection-age and the cancer-age across-seed median are transmission-floor-limited (not tightened); the total cancer count carries a small (~−1%, within the across-seed noise floor) competing-risk residual within the ±8% gate, which does NOT skew the age distributions; and and the per-genotype cancer split shifts modestly in multi-genotype runs (each ledger sub-cancer is keyed by source uid + sub-resolution index and a sim-shared registry enforces one cancer per sub-individual across genotypes, mirroring the agent-level cross-genotype cancellation; a small residual remains because the competition is resolved by earliest realized onset rather than the full severity-coupled dynamics).
