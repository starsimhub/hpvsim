...
- Adds **multiscale agents** via the `ms_agent_ratio` parameter (`hpv.Sim(ms_agent_ratio=N)`; default `1` = feature off). Ports v2's multiscale technique to the v3/Starsim architecture as a **cancer-pathway ledger**: each CIN agent's cancer pathway is resolved at `N` sub-resolutions — the agent's own cancer (counted at weight `1/N`) plus `N−1` EXTRA sub-cancers resolved as scheduled DATA in a deterministic side-RNG ledger (no fine People agents are grown). So the rare cancer-pathway events (causal infection, CIN2+, cancer) carry `N`× more samples per run for better-resolved age distributions (methods-manuscript Fig 5) at low agent count.
  ...
  - Scope/known residuals (documented in the design doc and `tests/test_multiscale_distribution.py`): multiscaling covers the cancer pathway only; causal-infection-age and the cancer-age across-seed median are transmission-floor-limited (not tightened); the total cancer count carries a small (~−5%) competing-risk residual (all of a coarse agent's extras are gated on that one body's survival, over-suppressing late-onset extras) within the ±8% gate, which does NOT skew the age distributions; and the per-genotype cancer split shifts modestly in multi-genotype runs (the ledger extras skip the cross-genotype cancer competition that the agents' own cancers still undergo).
