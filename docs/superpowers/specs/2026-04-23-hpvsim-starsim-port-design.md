...
|---|---|---|---|
| M0 | **Foundation** | `v3.0-dev` branch exists with CI green on a stub sim; `KICKOFF_DISCUSSION.md` + `MIGRATION_PLAN.md` committed; v2.x baseline outputs stored; anchor-scenario script and ±10% comparison script committed | CI passes; regression harness compares a v3 run to a stored v2 baseline and reports diffs |
