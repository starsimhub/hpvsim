...
- **Confirm Task 4b network exclusion is in effect** (fine agents must not transmit). Task 4b reduced the gross +131% inflation to a residual −16.6% and left `test_split_does_not_inflate_infections_via_network` as `xfail(strict=True)`. Task 6 must drive that test to pass (remove the xfail) by resolving the two measured residual mechanisms:
  - **(a) `new_infections` counted unscaled.** `ss.Infection.update_results` tallies `new_infections` via `np.count_nonzero(ti_infected==ti)` — a raw agent count, so a fine agent (scale 1/ratio) is counted as a full infection (a ti=0 spike when the init-time split copies `infected=True` onto fine agents). FIX: scale-weight the new-infection tally (override `update_results` in `HPV` to sum `people.scale[newly_infected]` instead of counting). This is the same results-audit pattern as Tasks 2/3 and is the cleanest first fix.
  - **(b) Transmission-timing deficit.** A cancer-bound coarse agent is shrunk + network-excluded at the cancer *decision* (set_prognoses/infection time), so it stops transmitting earlier than at ratio=1 (where it keeps transmitting through its CIN duration until cancer onset). Plus a small leak: ~63 agents that became fine *after* already being `participant=True` keep their edges (`set_network_states` only adds participants, never removes them). Compare against v2's timing (when does v2 drop the agent from `level0`?) and decide whether to (i) defer network exclusion of a splitting agent until its actual cancer onset, and/or (ii) actively remove newly-fine agents from existing edges. This is the genuine dynamics subtlety; use it as the final lever after (a) and the resample-p_cancer item.
