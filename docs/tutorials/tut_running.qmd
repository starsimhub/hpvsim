...
Click [here](https://mybinder.org/v2/gh/starsimhub/hpvsim/HEAD?urlpath=lab%2Ftree%2Fdocs%2Ftutorials%2Ftut_running.ipynb) to open an interactive version of this notebook.

</div>

## Multi-seed runs and scenario sweeps

HPVsim uses Starsim's native multi-sim machinery — `ss.MultiSim`,
`ss.parallel`, and Sciris's `sc.parallelize` — for replicate runs,
scenario comparisons, and parameter sweeps.
...
builds one sim per scenario, and pass the labeled sims to `ss.parallel`:
