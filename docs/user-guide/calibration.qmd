...
Mismatch is a weighted sum over columns. Each column's error is normalised by the largest observed value in it, so a cancer count in the thousands and a prevalence of 0.1 contribute comparably; `weights={'all_hpv.prevalence': 2.0}` then adjusts deliberately. Because normalisation depends on the data you supplied, mismatch values are comparable within one calibration and meaningless between two.
...
Six trials over two parameters is a plumbing test, not a calibration. For real work: fit the sexual network to behavioural data first, then transmission to prevalence, then progression to cancer, adding parameters only where the data can identify them. Expect several hundred to a few thousand trials, and run those on a machine you are not also using for anything else.
