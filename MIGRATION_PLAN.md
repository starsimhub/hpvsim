...
- **Post-merge sub-task (added 2026-05-28; oversight correction):** Expose all v2-shipped demographic locations by removing the `_KNOWN_LOCATIONS = ['nigeria']` gate in `hpvsim/data/country.py`. M02 shipped Nigeria only; the bundled data files (`hpvsim/data/files/populations.obj` and siblings) already cover 298 UN WPP 2024 entries and the loaders work for all of them. Demographics only; network calibration stays location-agnostic per v2's shipped behavior. Lands in its own branch (`country-support`) off `m07-multisim`. See `docs/superpowers/specs/2026-05-28-country-support-design.md`.
...
| Location-name normalization + subnational regions | M4 | The `country-support` PR (2026-05-28) removed the `_KNOWN_LOCATIONS` allowlist and exposed all 298 v2-shipped UN WPP 2024 entries, with `get_country_aliases()` covering common variants (e.g., "Côte d'Ivoire" / "Cote d'Ivoire" / "Ivory Coast"). Remaining work for M4: extend alias coverage as calibration users surface gaps, and design a path to subnational regions (long-standing user request that the v2 country-level adapter blocked). Touches `hpv.data.country.load_country` and `hpv.Sim`'s location handling. |
